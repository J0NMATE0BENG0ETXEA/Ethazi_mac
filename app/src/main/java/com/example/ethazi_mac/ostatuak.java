...
public class ostatuak extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        EnviarRecibirDatos("http://172.22.28.130/ethazi_mac/selectostatuak.php");
    }
        ...
        for(int i=0;i<ja.length();i+=12){
            ...
            try {
                Ostatu ost = new Ostatu(ja.getString(i),ja.getString(i+1),ja.getString(i+2),ja.getString(i+3),ja.getString(i+4),ja.getString(i+5),
                        ja.getString(i+6),ja.getString(i+7),ja.getString(i+8),ja.getString(i+9),ja.getString(i+10),ja.getString(i+11));
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id==R.id.albergues) {
            EnviarRecibirDatos("http://172.22.28.130/ethazi_mac/selectalberge.php");
        }
        if (id==R.id.agroturismos) {
            EnviarRecibirDatos("http://172.22.28.130/ethazi_mac/selectagro.php");
        }
        if (id==R.id.campings) {
            EnviarRecibirDatos("http://172.22.28.130/ethazi_mac/selectcamping.php");
        }
        if (id==R.id.casasrurales) {
            EnviarRecibirDatos("http://172.22.28.130/ethazi_mac/selectcasasru.php");
        }
    ...
    public void ikusiOstatua(Ostatu ostaua, View view){
        ...
        i.putExtra("helbidea", ostaua.getHelbidea());
        startActivity(i);
